...
(function () {
    ...
    function attach(img, tag, titre) {
        var src = img.getAttribute('src') || '';
        if (!isImg(src)) return;
        var idx = images.length;
        images.push({ src: img.src, alt: img.alt || '', tag: tag || '', titre: titre || '' });
        img.addEventListener('click', function () { openAt(idx); });
    }

    /* Galerie swipeable */
    document.querySelectorAll('.galerie-slide').forEach(function (slide) {
        var img   = slide.querySelector('img.galerie-slide-media');
        if (!img) return;
    ...
    function show(item) {
        /* Réinitialise l'animation pour chaque changement */
    ...
    function prev() { if (current > 0) { current--; show(images[current]); updateNav(); } }
    function next() { if (current < images.length - 1) { current++; show(images[current]); updateNav(); } }
